...
//this is the mugshot position
	...
	void DrawHealthTicker(
		...
		for(int i=0;i<STB_BEATERSIZE;i++){
			int alf=(i&1)?128:255;
			fill(
				(   // 4 health colors: green, yellow, orange, and red;
				    // uses green instead of player color for "Healthy"
					cp.health>75?color(alf,0,240,10)
					:cp.health>50?color(alf,240,210,10)
					:cp.health>25?color(alf,240,100,0)
					:color(alf,220,0,0)
				),
